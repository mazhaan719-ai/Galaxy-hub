...
🚫 No spam, floods, or ping abuse
🔒 Keep privacy, no doxxing allowed
📢 No ads 
⚠️ No NSFW, illegal, or harmful stuff
🛑 Hate speech = warning or ban
📬 Report issues to our main server 
🔴10 warnings after that instant ban
]]
...
if IsTextChatService then
    ...
    for _, p in pairs(Player:GetPlayers()) do hookPlayer(p) end
...
InputBox.FocusLost:Connect(function(enter)
    ...
    if currentTab == "Global" then
        if IsTextChatService then TextChatService.TextChannels.RBXGeneral:SendAsync(txt)
        else ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(txt, "All") end
    else
        local target = TargetBox.Text
        if target ~= "" then
            local cmd = "/w " .. target .. " " .. txt
            if IsTextChatService then TextChatService.TextChannels.RBXGeneral:SendAsync(cmd)
            else ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(cmd, "All") end
            AddMessage(PrivateScroll, "To ["..target.."]", txt, Color3.fromRGB(255, 0, 255))
        end
    end
    InputBox.Text = ""
    task.wait()
